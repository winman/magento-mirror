...
Mage::helper('adminhtml')->__('Brands Manager'));
  ...
  public function editAction()
  {
	...
	Mage::log("to edit:".$brandsId);
	$brandsModelClass = Mage::getConfig()->getModelClassName('brands/brands');
	Mage::log("model class:".$brandsModelClass);
    $brandsModel = Mage::getModel('brands/brands');
	Mage::log("resource name: ".$brandsModel->getResourceName());
	$brandsModel->load($brandsId);
    if ($brandsModel->getId() || $brandsId == 0) {
	  ...
	  Mage::log("add content block");
	  $this->_addContent($this->getLayout()->createBlock('brands/adminhtml_brands_edit'));
	  Mage::log("add left block");
      $this->_addLeft($this->getLayout()->createBlock('brands/adminhtml_brands_edit_tabs'));
      $this->renderLayout();
    } else {
